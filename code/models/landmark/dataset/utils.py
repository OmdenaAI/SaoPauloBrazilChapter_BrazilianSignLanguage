...
def check_angle_type(angle_type: str):
    if angle_type not in ['rad', 'grad', 'func', 'normalized_rad', 'shifted_rad']:
        raise Exception("Parameter 'ange_type' should be one of ['rad', 'grad', 'func', 'normalized_rad', 'shifted_rad']")

def check_distance_type(distance_type: str):
    if distance_type not in ['dist', 'normalized_dist', 'shifted_dist']:
        raise Exception("Parameter 'distance_type' should be one of ['dist', 'normalized_dist', 'shifted_dist']")
...
def load_config(config: Union[str, Dict], config_name: str) -> Dict:
        if isinstance(config, str):
            ...
            raise ValueError(f"Parameter {config_name} must be either a file path or dictionary.")